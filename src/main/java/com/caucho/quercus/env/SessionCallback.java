...
public class SessionCallback extends Value {
  ...
  {
    _open.call(env, env.createString(savePath, null), env.createStringOld(sessionName));
  }

  public StringValue read(Env env, String id)
  {
    try {
      Value value = _read.call(env, env.createStringOld(id));
  ...
  {
    try {
      _write.call(env, env.createStringOld(id), env.createStringOld(value));
    } catch (RuntimeException e) {
      throw e;
    } catch (Throwable e) {
  ...
  {
    try {
      _destroy.call(env, env.createStringOld(id));
    } catch (RuntimeException e) {
      throw e;
    } catch (Throwable e) {
      log.log(Level.FINE, e.toString(), e);
